...
interface Asteroid {
  ...
  vertices: number[]; // jagged edge offsets
  rotation: number;
  rotSpeed: number;
}
...
function createAsteroid(x: number, y: number, sizeIndex: number): Asteroid {
...
function drawBullets(s: GameState): void {
...
function drawTitleScreen(s: GameState): void {
  drawStars(s);
  s.frame++;
